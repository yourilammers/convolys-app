...
def team_membership_details(request, team_slug, membership_id):
    ...
    if not can_edit_team_members and not editing_self:
        messages.error(
            request,
            _("Sorry, you don't have permission to access that page.")
        )
    ...
    return render(request, 'teams/team_membership_details.html', {
...
def remove_team_membership(request, team_slug, membership_id):
    ...
    if membership.role == ROLE_ADMIN:
        ...
        if admin_count == 1:
            ...
            messages.error(
                request,
                _('You cannot remove the only administrator from a team. '
    ...
    messages.success(
        request,
        _('{member} was removed from {team}.').format(member=membership.user.get_display_name(),
